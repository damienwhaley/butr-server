...
function configureGateway(appSettings) {
  ...
  _httpLogFileStream = fs.createWriteStream(_httpLogFile, { flags: "a", encoding: 'utf8', mode: 0o666 });

  _server.use(morgan('combined', { stream: _httpLogFileStream }));
...
function startGateway(appSettings) {
  ...
  if (cluster.isMaster) {
	...
	domainWorker.on('error', function(err) {
	...
	domainWorker.run(function() {
      ...
      _app.use(vhost(appSettings.Gateway.VirtualHost, _server));
