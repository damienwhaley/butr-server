  ...
  if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {	  
    if (!err){
      for(i = 0; i < jsonResult.rows.length; i++) {    	  
        if (_configPermission[jsonResult.rows[i].message_magic] === undefined
          || _configPermission[jsonResult.rows[i].message_magic] === null) {
...
function validateAppSettings(appSettings) {
  ...
  // Application Path
  if (appSettings.ApplicationPath === undefined ||
    appSettings.ApplicationPath === null ||
    appSettings.ApplicationPath === '') {
    var applicationPath = __dirname;
    appSettings.ApplicationPath = applicationPath.substring(0, applicationPath.length - 4);
  }
...
function printAppSettings() {
  ...
  butrLog.writeLog(butrConstants.LOG_LEVEL_DEBUG, 'appSettings.ApplicationPath='+_configApp.ApplicationPath);
