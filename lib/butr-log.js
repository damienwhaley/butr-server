...
var _currentPidFilePath = '';
var _settingsPidFilePath = '';
...
function configureLogging(appSettings) {
  ...
  if (appSettings.Logging.PidFilePath !== '') {
    _settingsPidFilePath = appSettings.Logging.PidFilePath;
  } else {
    _settingsPidFilePath = 'logs/butr.pid';
  }
  ...
  if (_settingsPidFilePath.length > 0) {
    if (_settingsPidFilePath.charAt(0) === '/' || _settingsPidFilePath.charAt(0) === '\\'
      || path_regex.test(_settingsPidFilePath)) {
      _currentPidFilePath = _settingsPidFilePath;
    }
    else {
      _currentPidFilePath = _appPath+_settingsPidFilePath;
    }
  }
};
...
function writeHttpLog(loggingMessage) {
  ...
  if (_currentHttpLogPath !== '') {
    var fileStream = fs.createWriteStream(_currentHttpLogPath, { flags: 'a', encoding: 'utf8', mode: 0o666 });
...
function writeMessageLog(messageIn, messageOut) {
  ...
  if (_currentMessageLogPath !== '') {
    var fileStream = fs.createWriteStream(_currentMessageLogPath, { flags: 'a', encoding: 'utf8', mode: 0o666 });
    var fileLog = new log('debug', fileStream);
...
/**
 * This writes the pid file to store the process id.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param pid
 * @api public
 */
function writePidFile(pid) {
  ...
  if (_currentPidFilePath !== '') {
    var fileStream = fs.createWriteStream(_currentPidFilePath, { flags: 'w', encoding: "utf8", mode: 0o666 });
    fileStream.write(pid.toString());
  }
};

/**
 * This removes the pid file to store the process id.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @api public
 */
function removePidFile() {
  ...
  if (_currentPidFilePath !== '') {
	fs.unlink(_currentPidFilePath, function () {});
  }
}
...
module.exports.writePidFile = writePidFile;
module.exports.removePidFile = removePidFile;