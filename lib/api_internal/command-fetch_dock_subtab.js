...
var butrApiJsonCommandFetchDockSubtab = require('../api_json/command-fetch_dock_subtab');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_dock_subtab command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchDockSubtab(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE,
	butrConstants.DATABASE_TABLE_SYSTEM_DOCK_SUBTABS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_dock_subtab.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_dock_subtab.uuid = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_uuid !== undefined
          ...
          outJson.fetch_dock_subtab.system_dock_type_uuid = jsonResult.rows[0].system_dock_type_uuid;
        }
        else {
          outJson.fetch_dock_subtab.dock_uuid = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_name_label !== undefined
          ...
          outJson.fetch_dock_subtab.system_dock_type_name_label = jsonResult.rows[0].system_dock_type_name_label;
        }
        else {
          outJson.fetch_dock_subtab.system_dock_type_name_label = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_display_label !== undefined
          ...
          outJson.fetch_dock_subtab.system_dock_type_display_label = jsonResult.rows[0].system_dock_type_display_label;
        }
        else {
          outJson.fetch_dock_subtab.system_dock_type_display_label = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_uuid !== undefined
          ...
          outJson.fetch_dock_subtab.security_client_type_uuid = jsonResult.rows[0].security_client_type_uuid;
        }
        else {
          outJson.fetch_dock_subtab.security_client_type_uuid = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_name_label !== undefined
          ...
          outJson.fetch_dock_subtab.security_client_type_name_label = jsonResult.rows[0].security_client_type_name_label;
        }
        else {
          outJson.fetch_dock_subtab.security_client_type_name_label = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_display_label !== undefined
          ...
          outJson.fetch_dock_subtab.security_client_type_display_label = jsonResult.rows[0].security_client_type_display_label;
        }
        else {
          outJson.fetch_dock_subtab.security_client_type_display_label = '';
        }
        
        if (jsonResult.rows[0].dock_tab_uuid !== undefined
	      && jsonResult.rows[0].dock_tab_uuid !== null
	      && jsonResult.rows[0].dock_tab_uuid !== '') {
	      outJson.fetch_dock_subtab.dock_tab_uuid = jsonResult.rows[0].dock_tab_uuid;
	    }
	    else {
	      outJson.fetch_dock_subtab.dock_tab_uuid = '';
	    }
	    
	    if (jsonResult.rows[0].dock_tab_name !== undefined
	      && jsonResult.rows[0].dock_tab_name !== null
	      && jsonResult.rows[0].dock_tab_name !== '') {
	      outJson.fetch_dock_subtab.dock_tab_name = jsonResult.rows[0].dock_tab_name;
	    }
	    else {
	      outJson.fetch_dock_subtab.dock_tab_name = '';
	    }
	      
	    if (jsonResult.rows[0].dock_tab_display_name !== undefined
	      && jsonResult.rows[0].dock_tab_display_name !== null
	      && jsonResult.rows[0].dock_tab_display_name !== '') {
	      outJson.fetch_dock_subtab.dock_tab_display_name = jsonResult.rows[0].dock_tab_display_name;
	    }
	    else {
	      outJson.fetch_dock_subtab.dock_tab_display_name = '';
	    }
        
        if (jsonResult.rows[0].subtab_name !== undefined
          && jsonResult.rows[0].subtab_name !== null
          && jsonResult.rows[0].subtab_name !== '') {
          outJson.fetch_dock_subtab.subtab_name = jsonResult.rows[0].subtab_name;
        }
        else {
          outJson.fetch_dock_subtab.subtab_name = '';
        }
        
        if (jsonResult.rows[0].display_name !== undefined
          && jsonResult.rows[0].display_name !== null
          && jsonResult.rows[0].display_name !== '') {
          outJson.fetch_dock_subtab.display_name = jsonResult.rows[0].display_name;
        }
          else {
          outJson.fetch_dock_subtab.display_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_dock_subtab.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_dock_subtab.description = '';
        }
        
        if (jsonResult.rows[0].weighting !== undefined
          && jsonResult.rows[0].weighting !== null
          && jsonResult.rows[0].weighting !== '') {
          outJson.fetch_dock_subtab.weighting = jsonResult.rows[0].weighting;
        }
        else {
          outJson.fetch_dock_subtab.weighting = '';
        }
        
        if (jsonResult.rows[0].picture_path !== undefined
          && jsonResult.rows[0].picture_path !== null
          && jsonResult.rows[0].picture_path !== '') {
          outJson.fetch_dock_subtab.picture_path = jsonResult.rows[0].picture_path;
        }
        else {
          outJson.fetch_dock_subtab.picture_path = '';
        }
          
        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null
          && jsonResult.rows[0].is_active !== '') {
          outJson.fetch_dock_subtab.is_active = jsonResult.rows[0].is_active;
        }
        else {
          outJson.fetch_dock_subtab.is_active = '';
        }
        ...
        if (jsonResult.rows[0].is_record_required !== undefined
          ...
          outJson.fetch_dock_subtab.is_record_required = jsonResult.rows[0].is_record_required;
        }
        else {
          outJson.fetch_dock_subtab.is_record_required = '';
        }
        
        if (jsonResult.rows[0].subtab_action !== undefined
          && jsonResult.rows[0].subtab_action !== null
          && jsonResult.rows[0].subtab_action !== '') {
          outJson.fetch_dock_subtab.subtab_action = jsonResult.rows[0].subtab_action;
        }
        else {
          outJson.fetch_dock_subtab.subtab_action = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchDockSubtab.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a dock subtab.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchDockSubtab(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchDockSubtab('"+butrDatabase.sanitiseSqlString(sessionUuid)
