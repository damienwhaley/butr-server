...
var butrApiJsonCommandFetchCountry = require('../api_json/command-fetch_country');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchCountry(sessionUuid, transactionUuid, messageInternal.uuid);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_DOCKS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    ...
    callbackFetchCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_country.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_country.uuid = '';
        }
        
        if (jsonResult.rows[0].country_name !== undefined
          && jsonResult.rows[0].country_name !== null
          && jsonResult.rows[0].country_name !== '') {
          outJson.fetch_country.country_name = jsonResult.rows[0].country_name;
        }
        else {
          outJson.fetch_country.country_name = '';
        }
        
        if (jsonResult.rows[0].display_name !== undefined
          && jsonResult.rows[0].display_name !== null
          && jsonResult.rows[0].display_name !== '') {
          outJson.fetch_country.display_name = jsonResult.rows[0].display_name;
        }
        else {
          outJson.fetch_country.display_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_country.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_country.description = '';
        }
        
        if (jsonResult.rows[0].country_code !== undefined
          && jsonResult.rows[0].country_code !== null
          && jsonResult.rows[0].country_code !== '') {
          outJson.fetch_country.country_code = jsonResult.rows[0].country_code;
        }
        else {
          outJson.fetch_country.country_code = '';
        }
        
        if (jsonResult.rows[0].alternate_code !== undefined
          && jsonResult.rows[0].alternate_code !== null
          && jsonResult.rows[0].alternate_code !== '') {
          outJson.fetch_country.alternate_code = jsonResult.rows[0].alternate_code;
        }
          else {
          outJson.fetch_country.alternate_code = '';
        }
        
        if (jsonResult.rows[0].weighting !== undefined
          && jsonResult.rows[0].weighting !== null
          && jsonResult.rows[0].weighting !== '') {
          outJson.fetch_country.weighting = jsonResult.rows[0].weighting;
        }
        else {
          outJson.fetch_country.weighting = '';
        }
          
        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null
          && jsonResult.rows[0].is_active !== '') {
          outJson.fetch_country.is_active = jsonResult.rows[0].is_active;
        }
        else {
          outJson.fetch_country.is_active = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchCountry.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a country.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchCountry(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_location_FetchCountry('"+butrDatabase.sanitiseSqlString(sessionUuid)
