...
var butrApiJsonCommandAddUser = require('../api_json/command-add_user');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddUser(sessionUuid, transactionUuid, messageInternal.global_title_uuid, messageInternal.first_name, messageInternal.last_name, messageInternal.username, messageInternal.preferred_global_language_uuid);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_USER_USERS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddUser(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddUser(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_user.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_user.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddUser.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlAddUser(sessionUuid, transactionUuid, globalTitleUuid, firstName, lastName, username, preferredGlobalLanguageUuid) {
  'use strict';
  
  return "CALL p_user_AddUser('"+butrDatabase.sanitiseSqlString(sessionUuid)
