...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyRole(sessionUuid, transactionUuid, messageInternal.uuid,
    messageInternal.role_name, messageInternal.partition_uuid, 
    messageInternal.description, messageInternal.is_active);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    ...
    callbackModifyRole(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyRole(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_role.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_role.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyRole.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @param roleName
 *   - String containing the role_name of the record.
 * @param partitionUuid
 *   - String containing the UUID for the parition for the record.
 * @param description
 ...
 * @api private
 */
function sqlModifyRole(sessionUuid, transactionUuid, tableUuid, roleName, paritionUuid,
  description, isActive) {
  'use strict';
  
  return "CALL p_security_ModifyRole('"+butrDatabase.sanitiseSqlString(sessionUuid)
