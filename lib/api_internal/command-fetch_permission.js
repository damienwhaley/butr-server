...
var butrApiJsonCommandFetchPermission = require('../api_json/command-fetch_permission');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_permission command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchPermission(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SECURITY_PERMISSIONS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_permission.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_permission.uuid = '';
        }
        
        if (jsonResult.rows[0].module_uuid !== undefined
          && jsonResult.rows[0].module_uuid !== null
          && jsonResult.rows[0].module_uuid !== '') {
          outJson.fetch_permission.module_uuid = jsonResult.rows[0].module_uuid;
        }
        else {
          outJson.fetch_permission.module_uuid = '';
        }
        
        if (jsonResult.rows[0].module_name !== undefined
          && jsonResult.rows[0].module_name !== null
          && jsonResult.rows[0].module_name !== '') {
          outJson.fetch_permission.module_name = jsonResult.rows[0].module_name;
        }
        else {
          outJson.fetch_permission.module_name = '';
        }
        
        if (jsonResult.rows[0].permission_name !== undefined
          && jsonResult.rows[0].permission_name !== null
          && jsonResult.rows[0].permission_name !== '') {
          outJson.fetch_permission.permission_name = jsonResult.rows[0].permission_name;
        }
        else {
          outJson.fetch_permission.permission_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_permission.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_permission.description = '';
        }
        
        if (jsonResult.rows[0].magic !== undefined
          && jsonResult.rows[0].magic !== null
          && jsonResult.rows[0].magic !== '') {
          outJson.fetch_permission.magic = jsonResult.rows[0].magic;
        }
          else {
          outJson.fetch_permission.magic = '';
        }
        
        if (jsonResult.rows[0].importance !== undefined
          && jsonResult.rows[0].importance !== null
          && !isNaN(jsonResult.rows[0].importance)) {
          outJson.fetch_permission.importance = jsonResult.rows[0].importance;
        }
        else {
          outJson.fetch_permission.importance = null;
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchPermission.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a permission.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchPermission(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_security_FetchPermission('"+butrDatabase.sanitiseSqlString(sessionUuid)
