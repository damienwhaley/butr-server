...
var butrApiJsonCommandListSecurityAuthenticationMethodConfigurations = require('../api_json/command-list_security_authentication_method_configurations');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_security_authentication_method_configurations command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListSecurityAuthenticationMethodConfigurations(sessionUuid, transactionUuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_CONFGIURATION_LOOKUP_SECURITY_AUTHENTICATION_METHODS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListSecurityAuthenticationMethodConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListSecurityAuthenticationMethodConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_security_authentication_method_configurations = {};
  outJson.list_security_authentication_method_configurations.total_count = 0;
  outJson.list_security_authentication_method_configurations.offset = 0;
  outJson.list_security_authentication_method_configurations.size = -1;
  outJson.list_security_authentication_method_configurations.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_security_authentication_method_configurations.total_count = jsonResult.rows[0]._total_count;
    	}
        ...
        outJson.list_security_authentication_method_configurations.offset = offset;
        outJson.list_security_authentication_method_configurations.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_security_authentication_method_configurations.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListSecurityAuthenticationMethodConfigurations.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for listing the global title configuration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api public
 */
function sqlListSecurityAuthenticationMethodConfigurations(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_configuration_ListSecurityAuthenticationMethod(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
