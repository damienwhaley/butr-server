...
var butrApiJsonCommandListRoles = require('../api_json/command-list_roles');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_roles command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListRoles(sessionUuid, transactionUuid);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE,
	butrConstants.DATABASE_TABLE_SECURITY_ROLES, '0',
	function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListRoles(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListRoles(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_roles = {};
  outJson.list_roles.total_count = 0;
  outJson.list_roles.offset = 0;
  outJson.list_roles.size = -1;
  outJson.list_roles.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_roles.total_count = jsonResult.rows[0]._total_count;
    	}

        for(var i = 0; i < jsonResult.rows.length; i++) {
          item = {};
          
          if (jsonResult.rows[i].role_name !== null
	        && jsonResult.rows[i].role_name !== undefined
	        && jsonResult.rows[i].role_name !== '') {
	        item.role_name = jsonResult.rows[i].role_name;
	      }
	      else {
	    	item.role_name = '';
	      }
        ...
        if (ordinal === 'role_name' && direction === 'ascending') {
          unsortedResults.sort(sortRoleNameAscending);
        } else if (ordinal === 'role_name' && direction === 'descending') {
          unsortedResults.sort(sortRoleNameDescending);
        } else if (ordinal === 'partition_name' && direction === 'ascending') {
        ...
        outJson.list_roles.offset = offset;
        outJson.list_roles.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_roles.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListRoles.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This takes compares two objects by their role_name member in ascending order.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param a
 ...
 * @api private
 */
function sortRoleNameAscending(a, b) {
  'use strict';
  ...
  if (a.role_name !== undefined && a.role_name !== null) {
	a_string = a.role_name.toLowerCase();
  }
  
  if (b.role_name !== undefined && b.role_name !== null) {
    b_string = b.role_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their role_name member in descending order.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param a
 *   - First object to compare.
 * @param b
 ...
 * @api private
 */
function sortRoleNameDescending(a, b) {
  'use strict';
...
/**
 * This builds the SQL query for listing the roles.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListRoles(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_security_ListRole('"+butrDatabase.sanitiseSqlString(sessionUuid)
