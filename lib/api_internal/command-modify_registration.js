...
// TODO: figure out how the api_private is handled (is it already encoded?)
...
var butrApiJsonCommandModifyRegistration = require('../api_json/command-modify_registration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_registration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyRegistration(sessionUuid, transactionUuid,
    messageInternal.uuid, messageInternal.api_public, messageInternal.api_private,
    messageInternal.security_client_type_uuid, messageInternal.description,
    messageInternal.is_active);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SECURITY_REGISTRATIONS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyRegistration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyRegistration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_registration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_registration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyRegistration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a registration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param apiPublic
 *   - String continaing the api_public for the record.
 * @param apiPrivate
 *   - String containing the api_private for the record.
 * @param securityClientTypeUuid
 *   - String containing the security_client_type_uuid for the record.
 * @param description
 ...
 * @api private
 */
function sqlModifyRegistration(sessionUuid, transactionUuid, tableUuid, apiPublic, apiPrivate,
  securityClientTypeUuid, description, isActive) {
  'use strict';
  
  return "CALL p_security_ModifyRegistration('"+butrDatabase.sanitiseSqlString(sessionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(transactionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(tableUuid)
    +"', '"+butrDatabase.sanitiseSqlString(apiPublic)
    +"', '"+butrDatabase.sanitiseSqlString(apiPrivate)
