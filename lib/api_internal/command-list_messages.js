...
var butrApiJsonCommandListMessages = require('../api_json/command-list_messages');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_messages command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListMessages(sessionUuid, transactionUuid);
  
  // Grab the global title configuration settings.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_MESSAGES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListMessages(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListMessages(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_messages = {};
  outJson.list_messages.total_count = 0;
  outJson.list_messages.offset = 0;
  outJson.list_messages.size = -1;
  outJson.list_messages.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_messages.total_count = jsonResult.rows[0]._total_count;
    	}

        for(var i = 0; i < jsonResult.rows.length; i++) {
          item = {};
          
          if (jsonResult.rows[i].message_name !== null
	        && jsonResult.rows[i].message_name !== undefined
	        && jsonResult.rows[i].message_name !== '') {
	        item.message_name = jsonResult.rows[i].message_name;
	      }
	      else {
	    	item.message_name = '';
	      }
        ...
        if (ordinal === 'message_name' && direction === 'ascending') {
          unsortedResults.sort(sortMessageNameAscending);
        } else if (ordinal === 'message_name' && direction === 'descending') {
          unsortedResults.sort(sortMessageNameDescending);
        } else if (ordinal === 'module_name' && direction === 'ascending') {
        ...
        outJson.list_messages.offset = offset;
        outJson.list_messages.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_messages.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListMessages.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for listing the messages.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListMessages(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_system_ListMessage('"+butrDatabase.sanitiseSqlString(sessionUuid)
