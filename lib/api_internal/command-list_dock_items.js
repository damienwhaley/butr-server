...
var butrApiJsonCommandListDockItems = require('../api_json/command-list_dock_items');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListDockItems(sessionUuid, transactionUuid);
  
  // Grab the dock items.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_DOCK_ITEMS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListDockItems(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListDockItems(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_dock_items = {};
  outJson.list_dock_items.total_count = 0;
  outJson.list_dock_items.offset = 0;
  outJson.list_dock_items.size = -1;
  outJson.list_dock_items.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_dock_items.total_count = jsonResult.rows[0]._total_count;
    	}
        ...
        for(var i = 0; i < jsonResult.rows.length; i++) {
          ...
          if (jsonResult.rows[i].dock_display_name !== null
	        && jsonResult.rows[i].dock_display_name !== undefined
	        && jsonResult.rows[i].dock_display_name !== '') {
	        item.dock_name = jsonResult.rows[i].dock_display_name;
	      }
	      else {
	    	item.dock_name = '';
	      }
              
          if (item.dock_name === '' && jsonResult.rows[i].dock_name !== null
        ...
        } else if (ordinal === 'item_name' && direction === 'ascending') {
          unsortedResults.sort(sortItemNameAscending);
        } else if (ordinal === 'item_name' && direction === 'descending') {
          unsortedResults.sort(sortItemNameDescending);
        ...
        outJson.list_dock_items.offset = offset;
        outJson.list_dock_items.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_dock_items.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListDockItems.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This takes compares two objects by their item_name member in ascending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing -1 for a < b, 1 for a > b, and 0 for equal.
 */
function sortItemNameAscending(a, b) {
  'use strict';
  ...
  if (a.item_name !== undefined && a.item_name !== null) {
	a_string = a.item_name.toLowerCase();
  }
  
  if (b.item_name !== undefined && b.item_name !== null) {
    b_string = b.item_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their item_name member in descending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing 1 for a < b, -1 for a > b, and 0 for equal.
 */
function sortItemNameDescending(a, b) {
  'use strict';
...
/**
 * This builds the SQL query for listing dock items.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListDockItems(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_system_ListDockItem('"+butrDatabase.sanitiseSqlString(sessionUuid)
