...
var butrApiJsonCommandModifyGlobalTitleConfiguration = require('../api_json/command-modify_global_title_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_global_title_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyGlobalTitleConfiguration(sessionUuid, transactionUuid, messageInternal.uuid,
	messageInternal.name_label, messageInternal.display_label, messageInternal.description, messageInternal.weighting, messageInternal.is_active);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_GLOBAL_TITLES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyGlobalTitleConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyGlobalTitleConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.modify_global_title_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_global_title_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_global_title_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyGlobalTitleConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlModifyGlobalTitleConfiguration(sessionUuid, transactionUuid, tableUuid, nameLabel,
  displayLabel, description, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_ModifyGlobalTitle('"+butrDatabase.sanitiseSqlString(sessionUuid)
