...
var butrApiJsonCommandFetchPartition = require('../api_json/command-fetch_partition');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_partition command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchPartition(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_PARTITIONS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchPartition(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchPartition(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_partition.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_partition.uuid = '';
        }
          
        if (jsonResult.rows[0].partition_name !== undefined
          && jsonResult.rows[0].partition_name !== null
          && jsonResult.rows[0].partition_name !== '') {
          outJson.fetch_partition.partition_name = jsonResult.rows[0].partition_name;
        }
        else {
          outJson.fetch_partition.partition_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_partition.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_partition.description = '';
        }

        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null) {
          if (jsonResult.rows[0].is_active === 0) {
            outJson.fetch_partition.is_active = 0;
          }
          else {
            outJson.fetch_partition.is_active = 1;
          }
        }
        else {
          outJson.fetch_partition.is_active = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchPartition.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a partition.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchPartition(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchPartition(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
