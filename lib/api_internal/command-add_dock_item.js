...
var butrApiJsonCommandAddDockItem = require('../api_json/command-add_dock_item');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddDockItem(sessionUuid,
	transactionUuid, messageInternal.dock_uuid,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    butrConstants.DATABASE_TABLE_SYSTEM_DOCK_ITEMS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_dock_item.uuid = jsonResult.rows[0]._table_uuid;
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddDockItem.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api public
 */
function sqlAddDockItem(sessionUuid, transactionUuid, dockUuid, systemDockTypeUuid,
  securityClientTypeUuid, itemName, displayName, description, weighting, icon,
  itemAction, isActive) {
  'use strict';
  
  return "CALL p_system_AddDockItem('"+butrDatabase.sanitiseSqlString(sessionUuid)
