...
var butrApiJsonCommandListTables = require('../api_json/command-list_tables');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_tables command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListTables(sessionUuid, transactionUuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_TABLES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListTables(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListTables(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_tables = {};
  outJson.list_tables.total_count = 0;
  outJson.list_tables.offset = 0;
  outJson.list_tables.size = -1;
  outJson.list_tables.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_tables.total_count = jsonResult.rows[0]._total_count;
    	}
    	  
        for(var i = 0; i < jsonResult.rows.length; i++) {
          item = {};
        
          if (jsonResult.rows[i].table_name !== null
            && jsonResult.rows[i].table_name !== undefined
            && jsonResult.rows[i].table_name !== '') {
            item.table_name = jsonResult.rows[i].table_name;
          }
          else {
        ...
        if (ordinal === 'table_name' && direction === 'ascending') {
          unsortedResults.sort(sortTableNameAscending);
        } else if (ordinal === 'table_name' && direction === 'descending') {
          unsortedResults.sort(sortTableNameDescending);
        }
        ...
        outJson.list_tables.offset = offset;
        outJson.list_tables.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_tables.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListTables.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This takes compares two objects by their table_name member in ascending order.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param a
 ...
 * @api private
 */
function sortTableNameAscending(a, b) {
  'use strict';
  ...
  if (a.table_name !== undefined && a.table_name !== null) {
	a_string = a.table_name.toLowerCase();
  }
  
  if (b.table_name !== undefined && b.table_name !== null) {
    b_string = b.table_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their table_name member in descending order.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param a
 *   - First object to compare.
 * @param b
 ...
 * @api private
 */
function sortTableNameDescending(a, b) {
  'use strict';
...
/**
 * This builds the SQL query for listing the tables.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListTables(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_system_ListTable(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
