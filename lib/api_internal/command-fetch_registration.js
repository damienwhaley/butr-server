...
// TODO: figure out how to handle the api_private (should I show it??)
...
var butrApiJsonCommandFetchRegistration = require('../api_json/command-fetch_registration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_registration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchRegistration(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SECURITY_REGISTRATIONS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchRegistration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchRegistration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_registration.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_registration.uuid = '';
        }
          
        if (jsonResult.rows[0].group_name !== undefined
          && jsonResult.rows[0].group_name !== null
          && jsonResult.rows[0].group_name !== '') {
          outJson.fetch_registration.api_public = jsonResult.rows[0].api_public;
        }
        else {
          outJson.fetch_registration.api_public = '';
        }
        
        if (jsonResult.rows[0].api_private !== undefined
          && jsonResult.rows[0].api_private !== null
          && jsonResult.rows[0].api_private !== '') {
          outJson.fetch_registration.api_private = jsonResult.rows[0].api_private;
        }
        else {
          outJson.fetch_registration.api_private = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_registration.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_registration.description = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_uuid !== undefined
	      ...
	      outJson.fetch_registration.security_client_type_uuid = jsonResult.rows[0].security_client_type_uuid;
	    }
	    else {
	      outJson.fetch_registration.security_client_type_uuid = '';
	    }

        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null) {
          if (jsonResult.rows[0].is_active === 0) {
            outJson.fetch_registration.is_active = 0;
          }
          else {
            outJson.fetch_registration.is_active = 1;
          }
        }
        else {
          outJson.fetch_registration.is_active = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchRegistration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a registration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchRegistration(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_security_FetchRegistration(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
