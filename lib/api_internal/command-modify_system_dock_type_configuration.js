...
var butrApiJsonCommandModifySystemDockTypeConfiguration = require('../api_json/command-modify_system_dock_type_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_system_dock_type_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifySystemDockTypeConfiguration(sessionUuid, transactionUuid, messageInternal.uuid,
	messageInternal.name_label, messageInternal.display_label, messageInternal.description, messageInternal.magic,
	messageInternal.weighting, messageInternal.is_active);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_DOCK_TYPES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifySystemDockTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifySystemDockTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.modify_system_dock_type_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_system_dock_type_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_system_dock_type_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifySystemDockTypeConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlModifySystemDockTypeConfiguration(sessionUuid, transactionUuid, tableUuid, nameLabel,
  displayLabel, description, magic, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_ModifySystemDockType('"+butrDatabase.sanitiseSqlString(sessionUuid)
