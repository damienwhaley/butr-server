...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddCountry(sessionUuid, transactionUuid,
	messageInternal.country_name, messageInternal.display_name, messageInternal.description,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_LOCATION_COUNTRIES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_country.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_country.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddCountry.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a country.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlAddCountry(sessionUuid, transactionUuid, countryName, displayName,
  description, countryCode, alternateCode, weighting, isActive) {
  'use strict';
  
  return "CALL p_location_AddCountry('"+butrDatabase.sanitiseSqlString(sessionUuid)
