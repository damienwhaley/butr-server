...
var butrApiJsonCommandModifySecurityAuthenticationMethodConfiguration = require('../api_json/command-modify_security_authentication_method_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_security_authentication_method_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifySecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid, messageInternal.uuid,
	messageInternal.name_label, messageInternal.display_label, messageInternal.description,
	messageInternal.magic, messageInternal.weighting, messageInternal.is_active);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFGIURATION_LOOKUP_SECURITY_AUTHENTICATION_METHODS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifySecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifySecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.modify_security_authentication_method_configuration = {};
  ...
  if (!err) {
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        // Got some results so grab the uuid of the record that was modified.
        
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_security_authentication_method_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_security_authentication_method_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifySecurityAuthenticationMethodConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a global title configuration option.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param magic
 *   - String containing the magc for the record.
 * @param weighting
 ...
 * @api private
 */
function sqlModifySecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid, tableUuid,
  nameLabel, displayLabel, description, magic, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_ModifySecurityAuthenticationMethod('"+butrDatabase.sanitiseSqlString(sessionUuid)
