 ...
 */
var butrApiJsonCommandListSystemLogTypeConfigurations = require('../api_json/command-list_system_log_type_configurations');
...
/**
 * This executes the list_system_log_type_configurations command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListSystemLogTypeConfigurations(sessionUuid, transactionUuid);
  
  // Grab the global title configuration settings.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_LOG_TYPES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListSystemLogTypeConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListSystemLogTypeConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_system_log_type_configurations = {};
  outJson.list_system_log_type_configurations.total_count = 0;
  outJson.list_system_log_type_configurations.offset = 0;
  outJson.list_system_log_type_configurations.size = -1;
  outJson.list_system_log_type_configurations.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_system_log_type_configurations.total_count = jsonResult.rows[0]._total_count;
    	}
        ...
        outJson.list_system_log_type_configurations.offset = offset;
        outJson.list_system_log_type_configurations.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_system_log_type_configurations.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListSystemLogTypeConfigurations.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for listing the system dock type configuration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListSystemLogTypeConfigurations(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_configuration_ListSystemLogType(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
