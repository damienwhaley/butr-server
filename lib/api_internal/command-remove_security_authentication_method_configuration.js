...
var butrApiJsonCommandRemoveSecurityAuthenticationMethodConfiguration = require('../api_json/command-remove_security_authentication_method_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the remove_security_authentication_method_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlRemoveSecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACED_PURGE, butrConstants.DATABASE_TABLE_CONFGIURATION_LOOKUP_SECURITY_AUTHENTICATION_METHODS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackRemoveSecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackRemoveSecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.remove_security_authentication_method_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.remove_security_authentication_method_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.remove_security_authentication_method_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandRemoveSecurityAuthenticationMethodConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for removing a title configuration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param tableUuid
 *   - String containing the UUID of the table for the record to be removed.
 * @returns string
 *   - contains the SQL query to be executed.
 * @api public
 */
function sqlRemoveSecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_configuration_RemoveSecurityAuthenticationMethod('"+butrDatabase.sanitiseSqlString(sessionUuid)
