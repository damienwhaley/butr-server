...
var butrApiJsonCommandRemoveGlobalLanguageConfiguration = require('../api_json/command-remove_global_language_configuration');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlRemoveGlobalLanguageConfiguration(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Remove the global language configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_GLOBAL_LANGUAGES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackRemoveGlobalLanguageConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackRemoveGlobalLanguageConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.remove_global_language_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.remove_global_language_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.remove_global_language_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandRemoveGlobalLanguageConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for removing a language configuration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlRemoveGlobalLanguageConfiguration(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_configuration_RemoveGlobalLanguage('"+butrDatabase.sanitiseSqlString(sessionUuid)
