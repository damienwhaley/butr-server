...
var butrApiJsonCommandRemoveGlobalTitleConfiguration = require('../api_json/command-remove_global_title_configuration');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlRemoveGlobalTitleConfiguration(sessionUuid, transactionUuid, messageInternal.uuid);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_GLOBAL_TITLES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    ...
    callbackRemoveGlobalTitleConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackRemoveGlobalTitleConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.remove_global_title_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.remove_global_title_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.remove_global_title_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandRemoveGlobalTitleConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlRemoveGlobalTitleConfiguration(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_configuration_RemoveGlobalTitle('"+butrDatabase.sanitiseSqlString(sessionUuid)
