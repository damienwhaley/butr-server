...
var butrApiJsonCommandFetchTable = require('../api_json/command-fetch_table');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_table command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchTable(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_TABLES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchTable(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchTable(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_table.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_table.uuid = '';
        }
          
        if (jsonResult.rows[0].table_name !== undefined
          && jsonResult.rows[0].table_name !== null
          && jsonResult.rows[0].table_name !== '') {
          outJson.fetch_table.table_name = jsonResult.rows[0].table_name;
        }
        else {
          outJson.fetch_table.table_name = '';
        }
        
        if (jsonResult.rows[0].current_id !== undefined
          && jsonResult.rows[0].current_id !== null
          && jsonResult.rows[0].current_id !== '') {
          outJson.fetch_table.current_id = jsonResult.rows[0].current_id;
        }
        else {
          outJson.fetch_table.current_id = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchTable.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a table.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchTable(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchTable(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
