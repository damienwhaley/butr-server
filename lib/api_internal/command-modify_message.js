...
var butrApiJsonCommandModifyMessage = require('../api_json/command-modify_message');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_message command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyMessage(sessionUuid, transactionUuid, messageInternal.uuid,
    messageInternal.message_name, messageInternal.magic, messageInternal.module_uuid, messageInternal.description, messageInternal.is_active);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SYSTEM_MESSAGES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyMessage(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyMessage(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_message.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_message.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyMessage.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a message.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param messageName
 *   - String containing the message_name of the record.
 * @param magic
 *   - String containing the magic for the record.
 * @param moduleUuid
 *   - String containing the UUID for the module for the record.
 * @param description
 ...
 * @api private
 */
function sqlModifyMessage(sessionUuid, transactionUuid, tableUuid, messageName, magic, moduleUuid, description, isActive) {
  'use strict';
  
  return "CALL p_system_ModifyMessage('"+butrDatabase.sanitiseSqlString(sessionUuid)
