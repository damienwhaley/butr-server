...
var butrApiJsonCommandAddPermission = require('../api_json/command-add_permission');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddPermission(sessionUuid, transactionUuid, messageInternal.module_uuid,
    messageInternal.permission_name, messageInternal.description, messageInternal.magic, messageInternal.importance);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SECURITY_PERMISSIONS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_permission.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_permission.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddPermission.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
  ...
  if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
    return butrApiJsonCommandAddPermisson.render(null, res, outJson, metaJson, sessionData);
  }
}
 ...
 * @api private
 */
function sqlAddPermission(sessionUuid, transactionUuid, moduleUuid,
  permissionName, description, magic, importance) {
  'use strict';
  
  return "CALL p_security_AddPermission('"+butrDatabase.sanitiseSqlString(sessionUuid)
