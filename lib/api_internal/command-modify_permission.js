...
var butrApiJsonCommandModifyPermission = require('../api_json/command-modify_permission');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the add_permission command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyPermission(sessionUuid, transactionUuid, messageInternal.uuid,
	messageInternal.module_uuid, messageInternal.permission_name, messageInternal.description,
	messageInternal.magic, messageInternal.importance);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SECURITY_PERMISSIONS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyPermission(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_permission.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_permission.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyPermission.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
  ...
  if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
    return butrApiJsonCommandModifyPermisson.render(null, res, outJson, metaJson, sessionData);
  }
}

/**
 * This builds the SQL query for adding a permission.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param tableUuid
 *   - String continaing the uuid for the record.
 * @param moduleUuid
 *   - String continaing the module_uuid for the record.
 * @param permissionName
 *   - String containing the permission_name for the record.
 * @param description
 *   - String containing the description for the record.
 * @param magic
 *   - String containing the magic for the record.
 * @param importance
 *   - Integer containing the importance for the record.
 * @returns string
 *   - contains the SQL query to be executed.
 * @api private
 */
function sqlModifyPermission(sessionUuid, transactionUuid, tableUuid, moduleUuid,
  permissionName, description, magic, importance) {
  'use strict';
  
  return "CALL p_security_ModifyPermission('"+butrDatabase.sanitiseSqlString(sessionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(transactionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(tableUuid)
    +"', '"+butrDatabase.sanitiseSqlString(moduleUuid)
    +"', '"+butrDatabase.sanitiseSqlString(permissionName)
    +"', '"+butrDatabase.sanitiseSqlString(description)
    +"', '"+butrDatabase.sanitiseSqlString(magic)
    +"', "+butrDatabase.sanitiseSqlInteger(importance, null)+");";
}
