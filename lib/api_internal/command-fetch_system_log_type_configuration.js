 ...
 */
var butrApiJsonCommandFetchSystemLogTypeConfiguration = require('../api_json/command-fetch_system_log_type_configuration');
var butrConstants = require('../butr-constants');
var butrDatabase = require('../butr-database');
var butrUtil = require('../butr-util');

/**
 * This executes the fetch_system_log_type_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchSystemLogTypeConfiguration(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_LOG_TYPES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchSystemLogTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchSystemLogTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.fetch_system_log_type_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_system_log_type_configuration.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_system_log_type_configuration.uuid = '';
        }
          
        if (jsonResult.rows[0].name_label !== undefined
          && jsonResult.rows[0].name_label !== null
          && jsonResult.rows[0].name_label !== '') {
          outJson.fetch_system_log_type_configuration.name_label = jsonResult.rows[0].name_label;
        }
        else {
          outJson.fetch_system_log_type_configuration.name_label = '';
        }
        
        if (jsonResult.rows[0].display_label !== undefined
          && jsonResult.rows[0].display_label !== null
          && jsonResult.rows[0].display_label !== '') {
          outJson.fetch_system_log_type_configuration.display_label = jsonResult.rows[0].display_label;
        }
        else {
          outJson.fetch_system_log_type_configuration.display_label = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_system_log_type_configuration.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_system_log_type_configuration.description = '';
        }
        
        if (jsonResult.rows[0].weighting !== undefined
          && jsonResult.rows[0].weighting !== null
          && !isNaN(jsonResult.rows[0].weighting)) {
          outJson.fetch_system_log_type_configuration.weighting = jsonResult.rows[0].weighting;
        }
        else {
          outJson.fetch_system_log_type_configuration.weighting = '';
        }
        
        if (jsonResult.rows[0].magic !== undefined
	      && jsonResult.rows[0].magic !== null
	      && jsonResult.rows[0].magic !== '') {
	      outJson.fetch_system_log_type_configuration.magic = jsonResult.rows[0].magic;
	    }
	    else {
	      outJson.fetch_system_log_type_configuration.magic = '';
	    }

        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null) {
          if (jsonResult.rows[0].is_active === 0) {
            outJson.fetch_system_log_type_configuration.is_active = 0;
          }
          else {
            outJson.fetch_system_log_type_configuration.is_active = 1;
          }
        }
        else {
          outJson.fetch_system_log_type_configuration.is_active = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchSystemLogTypeConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlFetchSystemLogTypeConfiguration(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_configuration_FetchSystemLogType(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
