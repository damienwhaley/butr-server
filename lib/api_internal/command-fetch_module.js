...
var butrApiJsonCommandFetchModule = require('../api_json/command-fetch_module');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_module command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchModule(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_MODULES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchModule(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchModule(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_module.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_module.uuid = '';
        }
          
        if (jsonResult.rows[0].module_name !== undefined
          && jsonResult.rows[0].module_name !== null
          && jsonResult.rows[0].module_name !== '') {
          outJson.fetch_module.module_name = jsonResult.rows[0].module_name;
        }
        else {
          outJson.fetch_module.name_label = '';
        }
        
        if (jsonResult.rows[0].magic !== undefined
          && jsonResult.rows[0].magic !== null
          && jsonResult.rows[0].magic !== '') {
          outJson.fetch_module.magic = jsonResult.rows[0].magic;
        }
        else {
          outJson.fetch_module.magic = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_module.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_module.description = '';
        }

        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null) {
          if (jsonResult.rows[0].is_active === 0) {
            outJson.fetch_module.is_active = 0;
          }
          else {
            outJson.fetch_module.is_active = 1;
          }
        }
        else {
          outJson.fetch_module.is_active = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchModule.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a module.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchModule(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchModule(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
