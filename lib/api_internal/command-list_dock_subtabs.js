...
var butrApiJsonCommandListDockSubtabs = require('../api_json/command-list_dock_subtabs');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_dock_subtabs command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE,
	butrConstants.DATABASE_TABLE_SYSTEM_DOCK_SUBTABS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListDockSubtabs(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListDockSubtabs(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_dock_subtabs = {};
  outJson.list_dock_subtabs.total_count = 0;
  outJson.list_dock_subtabs.offset = 0;
  outJson.list_dock_subtabs.size = -1;
  outJson.list_dock_subtabs.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        for(var i = 0; i < jsonResult.rows.length; i++) {
          ...
          if (jsonResult.rows[i].dock_tab_display_name !== null
	        && jsonResult.rows[i].dock_tab_display_name !== undefined
	        && jsonResult.rows[i].dock_tab_display_name !== '') {
	        item.dock_tab_display_name = jsonResult.rows[i].dock_tab_display_name;
	      }
	      else {
	    	item.dock_tab_display_name = '';
	      }
              
          if (item.dock_tab_display_name === '' && jsonResult.rows[i].dock_tab_name !== null
	        && jsonResult.rows[i].dock_tab_name !== undefined
	        && jsonResult.rows[i].dock_tab_name !== '') {
        	item.dock_tab_display_name = jsonResult.rows[i].dock_tab_name;
	      }         
        ...
        } else if (ordinal === 'dock_tab_display_name' && direction === 'ascending') {
          unsortedResults.sort(sortDockTabDisplayNameAscending);
        } else if (ordinal === 'dock_tab_display_name' && direction === 'descending') {
          unsortedResults.sort(sortDockTabDisplayNameDescending);
        ...
        } else if (ordinal === 'subtab_name' && direction === 'ascending') {
          unsortedResults.sort(sortSubtabNameAscending);
        } else if (ordinal === 'subtab_name' && direction === 'descending') {
          unsortedResults.sort(sortSubtabNameDescending);
        }
        ...
        outJson.list_dock_subtabs.offset = offset;
        outJson.list_dock_subtabs.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_dock_subtabs.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListDockSubtabs.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This takes compares two objects by their dock_tab_display_name member in ascending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing -1 for a < b, 1 for a > b, and 0 for equal.
 */
function sortDockTabDisplayNameAscending(a, b) {
  'use strict';
  ...
  if (a.dock_tab_display_name !== undefined && a.dock_tab_display_name !== null) {
	a_string = a.dock_tab_display_name.toLowerCase();
  }
  
  if (b.dock_tab_display_name !== undefined && b.dock_tab_display_name !== null) {
    b_string = b.dock_tab_display_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their dock_tab_display_name member in descending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing 1 for a < b, -1 for a > b, and 0 for equal.
 */
function sortDockTabDisplayNameDescending(a, b) {
  'use strict';
...
/**
 * This takes compares two objects by their subtab_name member in ascending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing -1 for a < b, 1 for a > b, and 0 for equal.
 */
function sortSubtabNameAscending(a, b) {
  'use strict';
  ...
  if (a.subtab_name !== undefined && a.subtab_name !== null) {
	a_string = a.subtab_name.toLowerCase();
  }
  
  if (b.subtab_name !== undefined && b.subtab_name !== null) {
    b_string = b.subtab_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their subtab_name member in descending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing 1 for a < b, -1 for a > b, and 0 for equal.
 */
function sortSubtabNameDescending(a, b) {
  'use strict';
...
/**
 * This builds the SQL query for listing dock subtabs.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListDockSubtabs(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_system_ListDockSubtab('"+butrDatabase.sanitiseSqlString(sessionUuid)
