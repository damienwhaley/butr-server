...
var butrApiJsonCommandModifyDockSubitem = require('../api_json/command-modify_dock_subitem');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_dock_subitem command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyDockSubitem(sessionUuid,
	transactionUuid, messageInternal.uuid, messageInternal.dock_item_uuid,
	messageInternal.system_dock_type_uuid, messageInternal.security_client_type_uuid,
	messageInternal.subitem_name, messageInternal.display_name, messageInternal.description,
	messageInternal.weighting, messageInternal.icon, messageInternal.subitem_action,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    butrConstants.DATABASE_TABLE_SYSTEM_DOCK_SUBITEMS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyDockSubitem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyDockSubitem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_dock_subitem.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_dock_subitem.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyDockSubitem.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a dock.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param tableUuid
 *   - String containing the table uuid.
 * @param dockItemUuid
 *   - String containing the uuid of the dock_item that the dock_subitem is associated with.
 ...
 * @param subitemName
 *   - String containing the subitem_name for the record.
 * @param displayName
 ...
 * @param icon
 *   - String containing the address of the icon for the dock.
 * @param subitemAction
 *   - String containing the subitem_action for the dock item.
 * @param isActive
 ...
 * @api private
 */
function sqlModifyDockSubitem(sessionUuid, transactionUuid, tableUuid, dockItemUuid, systemDockTypeUuid,
  securityClientTypeUuid, subitemName, displayName, description, weighting, icon,
  subitemAction, isActive) {
  'use strict';
  
  return "CALL p_system_ModifyDockSubitem('"+butrDatabase.sanitiseSqlString(sessionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(transactionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(tableUuid)
    +"', '"+butrDatabase.sanitiseSqlString(dockItemUuid)
    +"', '"+butrDatabase.sanitiseSqlString(systemDockTypeUuid)
    +"', '"+butrDatabase.sanitiseSqlString(securityClientTypeUuid)
    +"', '"+butrDatabase.sanitiseSqlString(subitemName)
    ...
    +", '"+butrDatabase.sanitiseSqlString(icon)
    +"', '"+butrDatabase.sanitiseSqlString(subitemAction)
