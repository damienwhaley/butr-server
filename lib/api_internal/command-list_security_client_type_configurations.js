...
var butrApiJsonCommandListSecurityClientTypeConfigurations = require('../api_json/command-list_security_client_type_configurations');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_security_client_type_configurations command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListSecurityClientTypeConfigurations(sessionUuid, transactionUuid);
  
  // Grab the global title configuration settings.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_DOCK_TYPES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListSecurityClientTypeConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListSecurityClientTypeConfigurations(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_security_client_type_configurations = {};
  outJson.list_security_client_type_configurations.total_count = 0;
  outJson.list_security_client_type_configurations.offset = 0;
  outJson.list_security_client_type_configurations.size = -1;
  outJson.list_security_client_type_configurations.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_security_client_type_configurations.total_count = jsonResult.rows[0]._total_count;
    	}
        ...
        outJson.list_security_client_type_configurations.offset = offset;
        outJson.list_security_client_type_configurations.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_security_client_type_configurations.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListSecurityClientTypeConfigurations.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for listing the security client type configuration.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListSecurityClientTypeConfigurations(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_configuration_ListSecurityClientType(@i_SessionUuid='"+butrDatabase.sanitiseSqlString(sessionUuid)
