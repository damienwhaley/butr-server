...
var butrApiJsonCommandAddSecurityAuthenticationMethodConfiguration = require('../api_json/command-add_security_authentication_method_configuration');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  // DEBUG
  console.dir(messageInternal);
  
  var sqlString = sqlAddSecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid,
  ...
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFGIURATION_LOOKUP_SECURITY_AUTHENTICATION_METHODS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddSecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddSecurityAuthenticationMethodConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.add_security_authentication_method_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_security_authentication_method_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_security_authentication_method_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddSecurityAuthenticationMethodConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a security authentication method configuration option.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlAddSecurityAuthenticationMethodConfiguration(sessionUuid, transactionUuid, nameLabel,
  displayLabel, description, magic, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_AddSecurityAuthenticationMethod('"+butrDatabase.sanitiseSqlString(sessionUuid)
