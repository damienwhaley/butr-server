...
var butrApiJsonCommandAddDockSubitem = require('../api_json/command-add_dock_subitem');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the add_dock_subitem command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddDockSubitem(sessionUuid,
	transactionUuid, messageInternal.dock_item_uuid,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    butrConstants.DATABASE_TABLE_SYSTEM_DOCK_SUBITEMS, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddDockSubitem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddDockSubitem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_dock_subitem.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_dock_subitem.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddDockSubitem.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a dock subitem.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlAddDockSubitem(sessionUuid, transactionUuid, dockItemUuid, systemDockTypeUuid,
  securityClientTypeUuid, subitemName, displayName, description, weighting, icon,
  subitemAction, isActive) {
  'use strict';
  
  return "CALL p_system_AddDockSubitem('"+butrDatabase.sanitiseSqlString(sessionUuid)
