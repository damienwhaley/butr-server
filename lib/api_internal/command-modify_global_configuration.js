...
var butrApiInternalCommandModifyGlobalConfigurationSql = require('./sql/command-modify_global_configuration-sql');
var butrApiJsonCommandModifyGlobalConfiguration = require('../api_json/command-modify_global_configuration');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyGlobalConfiguration(sessionUuid, transactionUuid,
	messageInternal.uuid, messageInternal.name_label, messageInternal.display_label,
	messageInternal.magic, messageInternal.description, messageInternal.text_setting,
	messageInternal.integer_setting, messageInternal.float_setting,
	messageInternal.datetime_setting, messageInternal.uuid_setting,
	messageInternal.bit_setting);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_GLOBALS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyGlobalConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyGlobalConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.modify_global_configuration = {};
  ...
  if (!err) {
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        // Got some results so grab the uuid of the record updated.
        
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_global_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_global_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyGlobalConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a global configuration setting.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param tableUuid
 *   - String containing the UUID for the record.
 * @param nameLabel
 ...
 * @api private
 */
function sqlModifyGlobalConfiguration(sessionUuid, transactionUuid, tableUuid, nameLabel,
  displayLabel, magic, description, textSetting, integerSetting, floatSetting,
  datetimeSetting, uuidSetting, bitSetting) {
  'use strict';
  
  return "CALL p_configuration_ModifyGlobal('"+butrDatabase.sanitiseSqlString(sessionUuid)
