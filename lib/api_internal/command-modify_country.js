...
var butrApiJsonCommandModifyCountry = require('../api_json/command-modify_country');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_country command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyCountry(sessionUuid, transactionUuid,
	messageInternal.uuid, messageInternal.country_name, messageInternal.display_name, messageInternal.description,
	messageInternal.country_code, messageInternal.alternate_code, messageInternal.weighting,
	messageInternal.is_active);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_LOCATION_COUNTRIES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyCountry(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_country.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_country.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyCountry.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a country.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param tableUuid
 *   - String containing the UUID of the record to modify.
 * @param countryName
 *   - String containing the country_name for the record.
 * @param displayName
 *   - String containing the display_name for the record.
 * @param description
 *   - String containing the description for the record.
 * @param countryCode
 *   - String containing the country_code for the record.
 * @param alternateCode
 *   - String containing the alternate_code for the record.
 * @param weighting
 ...
 * @api private
 */
function sqlModifyCountry(sessionUuid, transactionUuid, tableUuid, countryName, displayName,
  description, countryCode, alternateCode, weighting, isActive) {
  'use strict';
  
  return "CALL p_location_ModifyCountry('"+butrDatabase.sanitiseSqlString(sessionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(transactionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(tableUuid)
    +"', '"+butrDatabase.sanitiseSqlString(countryName)
    +"', '"+butrDatabase.sanitiseSqlString(displayName)
    +"', '"+butrDatabase.sanitiseSqlString(description)
    +"', '"+butrDatabase.sanitiseSqlString(countryCode)
    +"', '"+butrDatabase.sanitiseSqlString(alternateCode)
