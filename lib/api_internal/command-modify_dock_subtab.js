...
var butrApiJsonCommandModifyDockSubtab = require('../api_json/command-modify_dock_subtab');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the modify_dock_subtab command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyDockSubtab(sessionUuid,
	...
	messageInternal.is_record_equired, messageInternal.is_active);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    ...
    callbackModifyDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_dock_subtab.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_dock_subtab.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyDockSubtab.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a dock subtab.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param dockTabUuid
 *   - String containing the uuid of the dock_tab.
 * @param subtabName
 ...
 * @api private
 */
function sqlModifyDockSubtab(sessionUuid, transactionUuid, tableUuid, securityClientTypeUuid,
  systemDockTypeUuid, dockTabUuid, subtabName, displayName, description, weighting, picturePath,
  subtabAction, isRecordRequired, isActive) {
  'use strict';
  
  return "CALL p_system_ModifyDockSubtab('"+butrDatabase.sanitiseSqlString(sessionUuid)
