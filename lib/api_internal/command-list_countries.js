...
var butrApiJsonCommandListCountries = require('../api_json/command-list_countries');
var butrUtil = require('../butr-util');
var butrConfig = require('../butr-config');

/**
 * This executes the list_countries command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlListCountries(sessionUuid, transactionUuid);
  
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_LOCATION_COUNTRIES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackListCountries(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, messageInternal.offset, messageInternal.size, messageInternal.direction, messageInternal.ordinal);
  });
 ...
 * @api private
 */
function callbackListCountries(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson, offset, size, direction, ordinal) {
  'use strict';
  ...
  outJson.list_countries = {};
  outJson.list_countries.total_count = 0;
  outJson.list_countries.offset = 0;
  outJson.list_countries.size = -1;
  outJson.list_countries.items = [];
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
    	...
    	if (jsonResult.rows[0]._total_count !== null
    	  ...
    	  outJson.list_countries.total_count = jsonResult.rows[0]._total_count;
    	}
        ...
        if (ordinal === 'country_name' && direction === 'ascending') {
        ...
        outJson.list_countries.offset = offset;
        outJson.list_countries.size = size;
        ...
        for (var i = parseInt(offset, 10); i < parseInt(offset, 10)+parseInt(size, 10); i++) {
          if (i < unsortedResults.length) {
            outJson.list_countries.items.push(unsortedResults[i]);
          }
          else {
        	break;
          }
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandListCountries.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for listing the countries.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlListCountries(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_location_ListCountry('"+butrDatabase.sanitiseSqlString(sessionUuid)
