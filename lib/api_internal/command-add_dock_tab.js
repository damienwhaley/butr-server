 ...
 */
var butrApiJsonCommandAddDockTab = require('../api_json/command-add_dock_tab');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddDockTab(sessionUuid,
	transactionUuid, messageInternal.security_client_type_uuid,
	messageInternal.system_dock_type_uuid, messageInternal.dock_item_uuid,
	messageInternal.dock_subitem_uuid, messageInternal.tab_name,
	messageInternal.display_name, messageInternal.description,
	messageInternal.weighting, messageInternal.icon, messageInternal.tab_action,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    ...
    callbackAddDockTab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddDockTab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_dock_tab.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_dock_tab.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddDockTab.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @param dockItemUuid
 *   - String containing the uuid of the dock_item that the dock_tab is associated with.
 * @param dockSubitemUuid
 *   - String containing the uuid of the dock_subitem that the dock_tab is accosiated with.
 * @param tabName
 ...
 * @param icon
 *   - String containing the address of the icon for the dock tab.
 * @param tabAction
 *   - String containing the stab_action for the dock tab.
 * @param isActive
 ...
 * @api private
 */
function sqlAddDockTab(sessionUuid, transactionUuid, securityClientTypeUuid,
  systemDockTypeUuid, dockItemUuid, dockSubitemUuid, tabName, displayName,
  description, weighting, icon, tabAction, isActive) {
  'use strict';
  
  return "CALL p_system_AddDockTab('"+butrDatabase.sanitiseSqlString(sessionUuid)
    ...
    +"', '"+butrDatabase.sanitiseSqlString(dockSubitemUuid)
