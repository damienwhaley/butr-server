    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        for(var i = 0; i < jsonResult.rows.length; i++) {
          ...
          if (jsonResult.rows[i].dock_subitem_display_name !== null
  	        && jsonResult.rows[i].dock_subitem_display_name !== undefined
  	        && jsonResult.rows[i].dock_subitem_display_name !== '') {
  	        item.dock_subitem_name = jsonResult.rows[i].dock_subitem_display_name;
  	      }
  	      else {
  	    	item.dock_subitem_name = '';
  	      }
                
          if (item.dock_subitem_name === '' && jsonResult.rows[i].dock_subitem_name !== null
  	        && jsonResult.rows[i].dock_subitem_name !== undefined
  	        && jsonResult.rows[i].dock_subitem_name !== '') {
            item.dock_subitem_name = jsonResult.rows[i].dock_subitem_name;
  	      }
        ...
        } else if (ordinal === 'dock_subitem_name' && direction === 'ascending') {
          unsortedResults.sort(sortDockSubitemNameAscending);
        } else if (ordinal === 'dock_subitem_name' && direction === 'descending') {
          unsortedResults.sort(sortDockSubitemNameDescending);
...
/**
 * This takes compares two objects by their dock_subitem_name member in ascending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing -1 for a < b, 1 for a > b, and 0 for equal.
 */
function sortDockSubitemNameAscending(a, b) {
  'use strict';
  ...
  if (a.dock_subitem_name !== undefined && a.dock_subitem_name !== null) {
	a_string = a.dock_subitem_name.toLowerCase();
  }
  
  if (b.dock_subitem_name !== undefined && b.dock_subitem_name !== null) {
    b_string = b.dock_subitem_name.toLowerCase();
  }
...
/**
 * This takes compares two objects by their dock_subitem_name member in descending order.
 * @param a
 *   - First object to compare.
 * @param b
 *   - Second object to compare.
 ...
 *   - integer containing 1 for a < b, -1 for a > b, and 0 for equal.
 */
function sortDockSubitemNameDescending(a, b) {
  'use strict';
