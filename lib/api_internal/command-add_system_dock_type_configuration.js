...
var butrApiJsonCommandAddSystemDockTypeConfiguration = require('../api_json/command-add_system_dock_type_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the add_system_dock_type_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddSystemDockTypeConfiguration(sessionUuid, transactionUuid,
  ...
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_DOCK_TYPES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddSystemDockTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddSystemDockTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.add_system_dock_type_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_system_dock_type_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_system_dock_type_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddSystemDockTypeConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a system dock type configuration option.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlAddSystemDockTypeConfiguration(sessionUuid, transactionUuid,
  nameLabel, displayLabel, description, magic, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_AddSystemDockType('"+butrDatabase.sanitiseSqlString(sessionUuid)
