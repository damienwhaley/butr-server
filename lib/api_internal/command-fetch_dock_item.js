...
var butrApiJsonCommandFetchDockItem = require('../api_json/command-fetch_dock_item');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchDockItem(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_DOCK_ITEMS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_dock_item.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_dock_item.uuid = '';
        }
        
        if (jsonResult.rows[0].dock_uuid !== undefined
	      && jsonResult.rows[0].dock_uuid !== null
	      && jsonResult.rows[0].dock_uuid !== '') {
	      outJson.fetch_dock_item.dock_uuid = jsonResult.rows[0].dock_uuid;
        }
	    else {
	      outJson.fetch_dock_item.dock_uuid = '';
	    }
        
        if (jsonResult.rows[0].dock_name !== undefined
          && jsonResult.rows[0].dock_name !== null
          && jsonResult.rows[0].dock_name !== '') {
          outJson.fetch_dock_item.dock_name = jsonResult.rows[0].dock_name;
        }
        else {
          outJson.fetch_dock_item.dock_name = '';
        }
      
        if (jsonResult.rows[0].dock_display_name !== undefined
          && jsonResult.rows[0].dock_display_name !== null
          && jsonResult.rows[0].dock_display_name !== '') {
          outJson.fetch_dock_item.dock_display_name = jsonResult.rows[0].dock_display_name;
        }
        else {
          outJson.fetch_dock_item.dock_display_name = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_uuid !== undefined
          ...
          outJson.fetch_dock_item.system_dock_type_uuid = jsonResult.rows[0].system_dock_type_uuid;
        }
        else {
          outJson.fetch_dock_item.dock_uuid = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_name_label !== undefined
          ...
          outJson.fetch_dock_item.system_dock_type_name_label = jsonResult.rows[0].system_dock_type_name_label;
        }
        else {
          outJson.fetch_dock_item.system_dock_type_name_label = '';
        }
        ...
        if (jsonResult.rows[0].system_dock_type_display_label !== undefined
          ...
          outJson.fetch_dock_item.system_dock_type_display_label = jsonResult.rows[0].system_dock_type_display_label;
        }
        else {
          outJson.fetch_dock_item.system_dock_type_display_label = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_uuid !== undefined
          ...
          outJson.fetch_dock_item.security_client_type_uuid = jsonResult.rows[0].security_client_type_uuid;
        }
        else {
          outJson.fetch_dock_item.security_client_type_uuid = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_name_label !== undefined
          ...
          outJson.fetch_dock_item.security_client_type_name_label = jsonResult.rows[0].security_client_type_name_label;
        }
        else {
          outJson.fetch_dock_item.security_client_type_name_label = '';
        }
        ...
        if (jsonResult.rows[0].security_client_type_display_label !== undefined
          ...
          outJson.fetch_dock_item.security_client_type_display_label = jsonResult.rows[0].security_client_type_display_label;
        }
        else {
          outJson.fetch_dock_item.security_client_type_display_label = '';
        }
        
        if (jsonResult.rows[0].item_name !== undefined
          && jsonResult.rows[0].item_name !== null
          && jsonResult.rows[0].item_name !== '') {
          outJson.fetch_dock_item.item_name = jsonResult.rows[0].item_name;
        }
        else {
          outJson.fetch_dock_item.item_name = '';
        }
        
        if (jsonResult.rows[0].display_name !== undefined
          && jsonResult.rows[0].display_name !== null
          && jsonResult.rows[0].display_name !== '') {
          outJson.fetch_dock_item.display_name = jsonResult.rows[0].display_name;
        }
          else {
          outJson.fetch_dock_item.display_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_dock_item.description = jsonResult.rows[0].description;
        }
        else {
          outJson.fetch_dock_item.description = '';
        }
        
        if (jsonResult.rows[0].weighting !== undefined
          && jsonResult.rows[0].weighting !== null
          && jsonResult.rows[0].weighting !== '') {
          outJson.fetch_dock_item.weighting = jsonResult.rows[0].weighting;
        }
        else {
          outJson.fetch_dock_item.weighting = '';
        }
        
        if (jsonResult.rows[0].picture_path !== undefined
          && jsonResult.rows[0].picture_path !== null
          && jsonResult.rows[0].picture_path !== '') {
          outJson.fetch_dock_item.picture_path = jsonResult.rows[0].picture_path;
        }
        else {
          outJson.fetch_dock_item.picture_path = '';
        }
          
        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null
          && jsonResult.rows[0].is_active !== '') {
          outJson.fetch_dock_item.is_active = jsonResult.rows[0].is_active;
        }
        else {
          outJson.fetch_dock_item.is_active = '';
        }
        
        if (jsonResult.rows[0].item_action !== undefined
          && jsonResult.rows[0].item_action !== null
          && jsonResult.rows[0].item_action !== '') {
          outJson.fetch_dock_item.item_action = jsonResult.rows[0].item_action;
        }
        else {
          outJson.fetch_dock_item.item_action = '';
        }

        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchDockItem.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a dock item.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchDockItem(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchDockItem('"+butrDatabase.sanitiseSqlString(sessionUuid)
