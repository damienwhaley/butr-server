 ...
 */
var butrApiJsonCommandAddDockSubtab = require('../api_json/command-add_dock_subtab');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddDockSubtab(sessionUuid,
	transactionUuid, messageInternal.security_client_type_uuid,
	messageInternal.system_dock_type_uuid, messageInternal.dock_tab_uuid,
	messageInternal.subtab_name, messageInternal.display_name, messageInternal.description,
	messageInternal.weighting, messageInternal.picture_path, messageInternal.subtab_action,
	messageInternal.is_record_required, messageInternal.is_active);
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    ...
    callbackAddDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddDockSubtab(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_dock_subtab.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_dock_subtab.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddDockSubtab.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for adding a dock subtab.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @param dockTabUuid
 *   - String containing the uuid of the dock tab.
 * @param subtabName
 *   - String containing the subtab_name for the record.
 * @param displayName
 ...
 * @param subtabAction
 *   - String containing the subtab_action for the dock tab.
 ...
 * @api private
 */
function sqlAddDockSubtab(sessionUuid, transactionUuid, securityClientTypeUuid,
  systemDockTypeUuid, dockTabUuid, subtabName, displayName, description,
  weighting, picturePath, subtabAction, isRecordRequired, isActive) {
  'use strict';
  
  return "CALL p_system_AddDockSubtab('"+butrDatabase.sanitiseSqlString(sessionUuid)
    ...
    +"', '"+butrDatabase.sanitiseSqlString(dockTabUuid)
    +"', '"+butrDatabase.sanitiseSqlString(subtabName)
    ...
    +"', '"+butrDatabase.sanitiseSqlString(subtabAction)
