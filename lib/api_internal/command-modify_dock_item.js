...
var butrApiJsonCommandModifyDockItem = require('../api_json/command-modify_dock_item');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the add_dock command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyDockItem(sessionUuid,
	transactionUuid, messageInternal.uuid, messageInternal.dock_uuid,
	messageInternal.system_dock_type_uuid, messageInternal.security_client_type_uuid,
	messageInternal.item_name, messageInternal.display_name, messageInternal.description,
	messageInternal.weighting, messageInternal.icon, messageInternal.item_action,
  ...
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE,
    butrConstants.DATABASE_TABLE_SYSTEM_DOCK_ITEMS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyDockItem(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_dock_item.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_dock_item.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyDockItem.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @param dockUuid
 *   - String containing the uuid of the dock that the dock_item is associated with.
 ...
 * @param itemName
 *   - String containing the item_name for the record.
 * @param displayName
 ...
 * @param itemAction
 *   - String containing the item_action for the dock item.
 * @param isActive
 ...
 * @api private
 */
function sqlModifyDockItem(sessionUuid, transactionUuid, tableUuid, dockUuid, systemDockTypeUuid,
  securityClientTypeUuid, itemName, displayName, description, weighting, icon,
  itemAction, isActive) {
  'use strict';
  
  return "CALL p_system_ModifyDockItem('"+butrDatabase.sanitiseSqlString(sessionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(transactionUuid)
    +"', '"+butrDatabase.sanitiseSqlString(tableUuid)
    +"', '"+butrDatabase.sanitiseSqlString(dockUuid)
    +"', '"+butrDatabase.sanitiseSqlString(systemDockTypeUuid)
    +"', '"+butrDatabase.sanitiseSqlString(securityClientTypeUuid)
    +"', '"+butrDatabase.sanitiseSqlString(itemName)
    ...
    +"', '"+butrDatabase.sanitiseSqlString(itemAction)
