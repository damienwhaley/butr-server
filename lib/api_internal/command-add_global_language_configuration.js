...
var butrApiJsonCommandAddGlobalLanguageConfiguration = require('../api_json/command-add_global_language_configuration');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');
var butrReplication = require('../butr-replication');

/**
 * This executes the add_global_language_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlAddGlobalLanguageConfiguration(sessionUuid, transactionUuid,
  ...
  // Save the global language configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_GLOBAL_LANGUAGES, '0', function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackAddGlobalLanguageConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackAddGlobalLanguageConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.add_global_language_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.add_global_language_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.add_global_language_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandAddGlobalLanguageConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlAddGlobalLanguageConfiguration(sessionUuid, transactionUuid, nameLabel, displayLabel, description, languageCode, languageFamily, countryUuid, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_AddGlobalLanguage('"+butrDatabase.sanitiseSqlString(sessionUuid)
