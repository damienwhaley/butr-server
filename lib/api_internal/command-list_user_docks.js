    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        for(var i = 0; i < jsonResult.rows.length; i++) {
          ...
          if (jsonResult.rows[i].docks_uuid !== null
            && jsonResult.rows[i].docks_uuid !== undefined
            && jsonResult.rows[i].docks_uuid !== '') {
            dockUuid = jsonResult.rows[i].docks_uuid;
          } else {
        	dockUuid = '';
          }
      ...
      if (jsonResult.rows.length > 0) {
        ...
        for(var i = 0; i < jsonResult.rows.length; i++) {
          ...
          if (jsonResult.rows[i].dock_items_uuid !== null
            ...
            dockItemUuid = jsonResult.rows[i].dock_items_uuid;
          }
          else {
        	dockItemUuid = '';
          }
