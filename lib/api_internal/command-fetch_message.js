...
var butrApiJsonCommandFetchMessage = require('../api_json/command-fetch_message');
var butrUtil = require('../butr-util');
var butrDatabase = require('../butr-database');

/**
 * This executes the fetch_message command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlFetchMessage(sessionUuid, transactionUuid, messageInternal.uuid);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_STORE, butrConstants.DATABASE_TABLE_SYSTEM_MESSAGES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackFetchMessage(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackFetchMessage(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined
          ...
          outJson.fetch_message.uuid = jsonResult.rows[0]._table_uuid;
        }
        else {
          outJson.fetch_message.uuid = '';
        }
        
        if (jsonResult.rows[0].module_uuid !== undefined
          && jsonResult.rows[0].module_uuid !== null
          && jsonResult.rows[0].module_uuid !== '') {
          outJson.fetch_message.module_uuid = jsonResult.rows[0].module_uuid;
        }
        else {
          outJson.fetch_message.module_uuid = '';
        }
        
        if (jsonResult.rows[0].module_name !== undefined
          && jsonResult.rows[0].module_name !== null
          && jsonResult.rows[0].module_name !== '') {
          outJson.fetch_message.module_name = jsonResult.rows[0].module_name;
        }
        else {
          outJson.fetch_message.module_name = '';
        }
        
        if (jsonResult.rows[0].message_name !== undefined
          && jsonResult.rows[0].message_name !== null
          && jsonResult.rows[0].message_name !== '') {
          outJson.fetch_message.message_name = jsonResult.rows[0].message_name;
        }
        else {
          outJson.fetch_message.message_name = '';
        }
        
        if (jsonResult.rows[0].description !== undefined
          && jsonResult.rows[0].description !== null
          && jsonResult.rows[0].description !== '') {
          outJson.fetch_message.description = jsonResult.rows[0].description;
        }
          else {
          outJson.fetch_message.description = '';
        }
        
        if (jsonResult.rows[0].magic !== undefined
          && jsonResult.rows[0].magic !== null
          && jsonResult.rows[0].magic !== '') {
          outJson.fetch_message.magic = jsonResult.rows[0].magic;
        }
        else {
          outJson.fetch_message.magic = '';
        }
          
        if (jsonResult.rows[0].is_active !== undefined
          && jsonResult.rows[0].is_active !== null
          && jsonResult.rows[0].is_active !== '') {
          outJson.fetch_message.is_active = jsonResult.rows[0].is_active;
        }
        else {
          outJson.fetch_message.is_active = 0;
        }

        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandFetchMessage.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for fetching a message.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlFetchMessage(sessionUuid, transactionUuid, tableUuid) {
  'use strict';
  
  return "CALL p_system_FetchMessage('"+butrDatabase.sanitiseSqlString(sessionUuid)
