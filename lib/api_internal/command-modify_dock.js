...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifyDock(sessionUuid, transactionUuid, messageInternal.uuid, messageInternal.security_client_type_uuid, messageInternal.dock_name, messageInternal.display_name, messageInternal.description, messageInternal.weighting, messageInternal.icon, messageInternal.is_active);
  
  // Save the global configuration setting.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SYSTEM_DOCKS, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifyDock(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifyDock(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_dock.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_dock.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifyDock.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for modifying a dock.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlModifyDock(sessionUuid, transactionUuid, tableUuid, securityClientTypeUuid, dockName, displayName, description, weighting, icon, isActive) {
  'use strict';
  
  return "CALL p_system_ModifyDock('"+butrDatabase.sanitiseSqlString(sessionUuid)
    ...
    +"', '"+butrDatabase.sanitiseSqlString(dockName)
