...
var butrApiJsonCommandEndSession = require('../api_json/command-end_session');
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlEndSession(sessionUuid, transactionUuid);
  
  // End the session in the database.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_SECURITY_SESSIONS, sessionUuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackEndSession(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackEndSession(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  if (!err) {
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        // Got some results so session has ended.
        ...
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandEndSession.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
...
/**
 * This builds the SQL query for ending the session.
 * @author Damien Whaley <damien@whalebonestudios.com>
 * @param sessionUuid
 ...
 * @api private
 */
function sqlEndSession(sessionUuid, transactionUuid) {
  'use strict';
  
  return "CALL p_security_EndSession('"+butrDatabase.sanitiseSqlString(sessionUuid)
