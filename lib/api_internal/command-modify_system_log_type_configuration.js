 ...
 */
var butrApiJsonCommandModifySystemLogTypeConfiguration = require('../api_json/command-modify_system_log_type_configuration');
...
/**
 * This executes the modify_system_log_type_configuration command.
...
function execute(res, metaJson, messageFormat, messageInternal, sessionData) {
  ...
  var sqlString = sqlModifySystemLogTypeConfiguration(sessionUuid, transactionUuid, messageInternal.uuid,
  ...
  // Execute the query.
  butrDatabase.executeSqlQuery(sqlString, butrConstants.DATABASE_CACHE_MEMCACHED_PURGE, butrConstants.DATABASE_TABLE_CONFIGURATION_LOOKUP_SYSTEM_LOG_TYPES, messageInternal.uuid, function (err, tableName, uuid, sqlQuery, jsonResult) {
    'use strict';
      
    callbackModifySystemLogTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson);
  });
 ...
 * @api private
 */
function callbackModifySystemLogTypeConfiguration(err, tableName, uuid, sqlQuery, jsonResult, res, metaJson, sessionData, messageFormat, outJson) {
  'use strict';
  ...
  outJson.modify_system_log_type_configuration = {};
    ...
    if (jsonResult !== undefined && jsonResult !== null && typeof(jsonResult) === 'object') {
      ...
      if (jsonResult.rows.length > 0) {
        ...
        if (jsonResult.rows[0]._table_uuid !== undefined && jsonResult.rows[0]._table_uuid !== null && jsonResult.rows[0]._table_uuid !== '') {
          outJson.modify_system_log_type_configuration.uuid = jsonResult.rows[0]._table_uuid;
          ...
          outJson.modify_system_log_type_configuration.uuid = '';
        }
        
        if(messageFormat === butrConstants.MESSAGE_FORMAT_JSON) {
          return butrApiJsonCommandModifySystemLogTypeConfiguration.render(null, res, outJson, metaJson, sessionData);
        }
      }
    }
  }
 ...
 * @api private
 */
function sqlModifySystemLogTypeConfiguration(sessionUuid, transactionUuid, tableUuid, nameLabel,
  displayLabel, description, magic, weighting, isActive) {
  'use strict';
  
  return "CALL p_configuration_ModifySystemLogType('"+butrDatabase.sanitiseSqlString(sessionUuid)
